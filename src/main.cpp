...
#if defined(NDEBUG)
# error "ANKACOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf5f0f246818e179c5bb3630f412d643601a3dc9cb46c21984842bb1b52339992");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // ANKACOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "ANKACOIN Signed Message:\n";
...
{
    // ANKACOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 179 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 175420000 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ANKACOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ANKACOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1520903428, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "milliyetcigazete 13 Mart 2018  President Erdogan Nato";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 179 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04fe7ca62273b889eeda631e42816142962d966e397b9ab3722e7d72e3f6fbfd59d6c69b4af967a3ab6f381f681990f23f2813558a8e1520698c309b8b9836004f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1520903428;
        ...
        assert(block.hashMerkleRoot == uint256("0x35ae0adb8aff2dc9719aa76ecb36fbefa2606221805f4eaa41b6b7251bc234d1"));
...
unsigned char pchMessageStart[4] = { 0x02, 0xab, 0x38, 0xb9 }; // ANKACOIN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ANKACOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ANKACOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ANKACOINMiner(CWallet *pwallet)
{
    printf("ANKACOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ankacoin-miner");
    ...
    try { loop {
        ...
        printf("Running ANKACOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ANKACOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ANKACOINMiner, pwallet));
}
